...
/**
 * Convert an ISO/IEC 646 byte encoded per ITU-T Rec. X.213 to a UTF-8 code point
 *
 * Returns a `null` if the byte does not encode a permitted character.
...
/**
 * Convert a UTF-8 code point to an ISO/IEC 646 byte per ITU-T Rec. X.213
 *
 * Returns `null` if the code point `c` is not a permitted character.
 * Permitted characters are those from ASCII code points 32 to 127, inclusively.
 *
 * Out of precaution, this library additionally prohibits code point 127
 * (`ESC`). This is a control character, and I think it was the ITU's
