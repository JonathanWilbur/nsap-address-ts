...
describe("X213NetworkAddress", () => {
    ...
    it("should return the correct ipv4 address", () => {
        const naddr = X213NetworkAddress.fromIpv4Address([255, 0, 10, 88]);
        const ip = naddr.getIpAddress();
    ...
    it("should return the correct ipv6 address", () => {
        ...
        const naddr = X213NetworkAddress.fromIpv6Address(input);
        const output = naddr.getIpAddress();
        assert.deepEqual(output, input);
        assert.equal(naddr.toString(), "IP6+1122:3344:5566:7788:99AA:BBCC:DDEE:FF00");
    });

    it("should return undefined from getIpAddress if the ip type is not recognized", () => {
        const bytes = new Uint8Array([
            AFI_IANA_ICP_BIN,
            0,
            2,
        ...
        const naddr = new X213NetworkAddress(bytes);
        const result = naddr.getIpAddress();
        assert.equal(result, undefined);
    });

    it("should return undefined from getIpAddress if the nsap length is not 20", () => {
        const bytes = new Uint8Array([
            AFI_IANA_ICP_BIN,
            0,
            2,
    ...
    it("should return the correct url", () => {
        ...
        const naddr1 = X213NetworkAddress.fromItotUrl(url);
        const output1 = naddr1.getUrl();
        assert.deepEqual(output1, url);

        const naddr2 = X213NetworkAddress.fromNonOsiUrl(url);
        const output2 = naddr2.getUrl();
        assert.deepEqual(output2, url);
    });

    it("should fail to return a url from getUrl if the nsap length is too short", () => {
        const url = "https://wildboarsoftware.com/x500directory";
    ...
    it("should return network socket info with port but no transport set", () => {
        const naddr = X213NetworkAddress.fromSocketAddress(3, [0xFF, 11, 0, 128], 65535);
        const info = naddr.getRfc1277Socket();
        assert.ok(info);
        const [ network, ip, port, tset ] = info;
        assert.equal(network, 3);
    ...
    it("should return network socket info with no port nor transport set", () => {
        const naddr = X213NetworkAddress.fromSocketAddress(3, [0xFF, 11, 0, 128]);
        const info = naddr.getRfc1277Socket();
        assert.ok(info);
        const [ network, ip, port, tset ] = info;
        assert.equal(network, 3);
    ...
    it("should not return network socket info if the nsap has a short dsp", () => {
        ...
        const info = naddr2.getRfc1277Socket();
        assert.ok(!info);
    });
    ...
    it("idiDigits() should return the correct digits when an odd-length idi is used with a decimal dsp", () => {
        ...
        // the last digit of the IDI. We therefore want to ensure that idiDigits()
